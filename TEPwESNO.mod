# Transmission Expansion Planning with Enhanced Seasonal Network Optimization
# TEPwESNO
...
subject to NLine_Flow_value{i in NEWLINE, t in HOUR, d in TDAY, y in EPOCH}:
                NL_flow[i,t,d,y] - (bus_angle[NL_fbus[i],t,d,y] - bus_angle[NL_tbus[i],t,d,y])/NL_x[i] <= BigNum*(1-zNLine[i,d,y]) ;
...
subject to Thermal_NLine{j in NEWLINE, t in HOUR, d in TDAY, y in EPOCH}:
				-NL_flow[j,t,d,y] <= NL_rateA[j]*zNLine[j,d,y];
...
option gurobi_options('mipgap=0.00001 timelim=3000 logfile=TEPwESNO_solverlog.txt');
